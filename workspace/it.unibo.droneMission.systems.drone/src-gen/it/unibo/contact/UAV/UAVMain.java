...
public abstract class UAVMain implements IContactSystem{
	...
	protected void initSupport(){
		MsgUtil.init(view);
		core = ((LindaLike)LindaLike.initSpace(view,"uAV"));
	}
	//For debug purpose
	...
	protected void configure(){
		...
		configureSubjects();  
			try {
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  UAV connecting ...",14);			
	 			ask_setConnChannel_space();
				serveUpdateDispatchThread();
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  UAV working ...",14);
			} catch (Exception e) {
	 			//e.printStackTrace();
	...
	System.out.println("UAV TERMINATES");
	try {
	...
	protected void ask_setConnChannel_space()  {
		try{
	  		ILindaLike support = PlatformExpert.findOutSupport("space","setConnChannel","uAV",view);
		 	RunTimeKb.addSubjectConnectionSupport("uAV", support, view );
		 	String msgOut = "space_setConnChannel(uAV, setConnChannel, connect, 0) "  ;
			support.out( msgOut );
	/*		
			...
			cr.start();
	*/	
			IAcquireOneReply answer = new AcquireOneReply("uAV", "space","setConnChannel",core, 
				"uAV"+"_space_setConnChannel(space,setConnChannel,M,0)",view);
	 ...
	 protected void serveUpdateDispatchThread() throws Exception {
			new Thread(){
				...
				protected IMessage hl_node_serve_update(   ) throws Exception {
					IMessage m = new Message("uAV"+"_update"+"(ANY,update,M,N)");
					IMessage inMsg = comSup.inOnly( "uAV" ,"update", m );				
					return inMsg;				
				}		
		
				public void run(){
					System.out.println("uAV serveUpdateDispatchThread started");
					while(goon)
					try {
						IMessage m =  hl_node_serve_update();
	 					System.out.println("uAV storing content of: " + m   );
