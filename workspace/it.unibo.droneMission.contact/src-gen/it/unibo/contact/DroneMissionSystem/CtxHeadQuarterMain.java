...
public class CtxHeadQuarterMain implements IContactSystem{
	...
	protected void initSupport(){
		MsgUtil.init(view);
		core = ((LindaLike)LindaLike.initSpace(view,"ctxHeadQuarter"));
	}
	protected void initGui(){
	 env = new EnvFrame( "CtxHeadQuarter", this );
	 env.init();
	 env.writeOnStatusBar( Thread.currentThread() +  " | CtxHeadQuarter working ...",14);
	...
	protected void configure(){
		...
		configureSubjects();  
			try {
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  ctxHeadQuarter connecting ...",14);			
	 			ask_setConnChannel_space();
				serveUpdateDispatchThread();
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  ctxHeadQuarter working ...",14);
			} catch (Exception e) {
	 			//e.printStackTrace();
	...
	System.out.println("CtxHeadQuarter TERMINATES");
	try {
	...
	protected void ask_setConnChannel_space()  {
		try{
	  		ILindaLike support = PlatformExpert.findOutSupport("space","setConnChannel","ctxHeadQuarter",view);
		 	RunTimeKb.addSubjectConnectionSupport("ctxHeadQuarter", support, view );
		 	String msgOut = "space_setConnChannel(ctxHeadQuarter, setConnChannel, connect, 0) "  ;
			support.out( msgOut );
	/*		
			...
			cr.start();
	*/	
			IAcquireOneReply answer = new AcquireOneReply("ctxHeadQuarter", "space","setConnChannel",core, 
				"ctxHeadQuarter"+"_space_setConnChannel(space,setConnChannel,M,0)",view);
	 ...
	 protected void serveUpdateDispatchThread() throws Exception {
			new Thread(){
				...
				protected IMessage hl_node_serve_update(   ) throws Exception {
					IMessage m = new Message("ctxHeadQuarter"+"_update"+"(ANY,update,M,N)");
					IMessage inMsg = comSup.inOnly( "ctxHeadQuarter" ,"update", m );				
					return inMsg;				
				}		
		
				public void run(){
					System.out.println("ctxHeadQuarter serveUpdateDispatchThread started");
					while(goon)
					try {
						IMessage m =  hl_node_serve_update();
	 					System.out.println("ctxHeadQuarter storing content of: " + m   );
	...
	CtxHeadQuarterMain system = new CtxHeadQuarterMain( );
	system.doJob();
	}
}//CtxHeadQuarterSupportMain
