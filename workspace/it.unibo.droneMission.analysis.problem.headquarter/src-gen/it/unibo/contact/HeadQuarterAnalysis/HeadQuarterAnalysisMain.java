...
public abstract class HeadQuarterAnalysisMain implements IContactSystem{
...
protected HeadQuarterAnalysisObserver observer;
	public void doJob(){
		initProperty();
		init();
		configure();
	...
	protected void initSupport(){
		MsgUtil.init(view);
		core = ((LindaLike)LindaLike.initSpace(view,"headQuarterAnalysis"));
	}
	//For debug purpose
	...
	protected void configure(){
		...
		configureSubjects();  
			try {
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  HeadQuarterAnalysis connecting ...",14);			
	 			ask_setConnChannel_space();
				serveUpdateDispatchThread();
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  HeadQuarterAnalysis working ...",14);
			} catch (Exception e) {
	 			//e.printStackTrace();
	...
	protected void registerObservers(){
			try {
				observer = new HeadQuarterAnalysisObserver();
	...
	System.out.println("HeadQuarterAnalysis TERMINATES");
	try {
	 	observer.terminate();
	...
	protected void ask_setConnChannel_space()  {
		try{
	  		ILindaLike support = PlatformExpert.findOutSupport("space","setConnChannel","headQuarterAnalysis",view);
		 	RunTimeKb.addSubjectConnectionSupport("headQuarterAnalysis", support, view );
		 	String msgOut = "space_setConnChannel(headQuarterAnalysis, setConnChannel, connect, 0) "  ;
			support.out( msgOut );
	/*		
			...
			cr.start();
	*/	
			IAcquireOneReply answer = new AcquireOneReply("headQuarterAnalysis", "space","setConnChannel",core, 
				"headQuarterAnalysis"+"_space_setConnChannel(space,setConnChannel,M,0)",view);
	 ...
	 protected void serveUpdateDispatchThread() throws Exception {
			new Thread(){
				...
				protected IMessage hl_node_serve_update(   ) throws Exception {
					IMessage m = new Message("headQuarterAnalysis"+"_update"+"(ANY,update,M,N)");
					IMessage inMsg = comSup.inOnly( "headQuarterAnalysis" ,"update", m );				
					return inMsg;				
				}		
		
				public void run(){
					System.out.println("headQuarterAnalysis serveUpdateDispatchThread started");
					while(goon)
					try {
						IMessage m =  hl_node_serve_update();
	 					System.out.println("headQuarterAnalysis storing content of: " + m   );
	...
	HeadQuarterAnalysis system = new HeadQuarterAnalysis( );
	system.doJob();
	}
}//HeadQuarterAnalysisSupportMain
