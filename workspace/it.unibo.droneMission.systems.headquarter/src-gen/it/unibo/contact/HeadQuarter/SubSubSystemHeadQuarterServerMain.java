...
public class SubSubSystemHeadQuarterServerMain implements IContactSystem{
	...
	protected void initSupport(){
		MsgUtil.init(view);
		core = ((LindaLike)LindaLike.initSpace(view,"subSubSystemHeadQuarterServer"));
	}
	protected void initGui(){
	 env = new EnvFrame( "SubSubSystemHeadQuarterServer", this );
	 env.init();
	 env.writeOnStatusBar( Thread.currentThread() +  " | SubSubSystemHeadQuarterServer working ...",14);
	...
	protected void configure(){
		...
		configureSubjects();  
			try {
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  subSubSystemHeadQuarterServer connecting ...",14);			
	 			ask_setConnChannel_space();
				serveUpdateDispatchThread();
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  subSubSystemHeadQuarterServer working ...",14);
			} catch (Exception e) {
	 			//e.printStackTrace();
	...
	System.out.println("SubSubSystemHeadQuarterServer TERMINATES");
	try {
	...
	protected void ask_setConnChannel_space()  {
		try{
	  		ILindaLike support = PlatformExpert.findOutSupport("space","setConnChannel","subSubSystemHeadQuarterServer",view);
		 	RunTimeKb.addSubjectConnectionSupport("subSubSystemHeadQuarterServer", support, view );
		 	String msgOut = "space_setConnChannel(subSubSystemHeadQuarterServer, setConnChannel, connect, 0) "  ;
			support.out( msgOut );
	/*		
			...
			cr.start();
	*/	
			IAcquireOneReply answer = new AcquireOneReply("subSubSystemHeadQuarterServer", "space","setConnChannel",core, 
				"subSubSystemHeadQuarterServer"+"_space_setConnChannel(space,setConnChannel,M,0)",view);
	 ...
	 protected void serveUpdateDispatchThread() throws Exception {
			new Thread(){
				...
				protected IMessage hl_node_serve_update(   ) throws Exception {
					IMessage m = new Message("subSubSystemHeadQuarterServer"+"_update"+"(ANY,update,M,N)");
					IMessage inMsg = comSup.inOnly( "subSubSystemHeadQuarterServer" ,"update", m );				
					return inMsg;				
				}		
		
				public void run(){
					System.out.println("subSubSystemHeadQuarterServer serveUpdateDispatchThread started");
					while(goon)
					try {
						IMessage m =  hl_node_serve_update();
	 					System.out.println("subSubSystemHeadQuarterServer storing content of: " + m   );
	...
	SubSubSystemHeadQuarterServerMain system = new SubSubSystemHeadQuarterServerMain( );
	system.doJob();
	}
}//SubSubSystemHeadQuarterServerSupportMain
