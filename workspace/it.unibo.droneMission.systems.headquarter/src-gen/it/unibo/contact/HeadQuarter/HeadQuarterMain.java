...
public abstract class HeadQuarterMain implements IContactSystem{
...
protected HeadQuarterObserver observer;
	public void doJob(){
		initProperty();
		init();
		configure();
	...
	protected void initSupport(){
		MsgUtil.init(view);
		core = ((LindaLike)LindaLike.initSpace(view,"headQuarter"));
	}
	//For debug purpose
	...
	protected void configure(){
		...
		configureSubjects();  
			try {
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  HeadQuarter connecting ...",14);			
	 			ask_setConnChannel_space();
				serveUpdateDispatchThread();
				if( env != null) env.writeOnStatusBar( Thread.currentThread() +  " |  HeadQuarter working ...",14);
			} catch (Exception e) {
	 			//e.printStackTrace();
	...
	protected void registerObservers(){
			try {
				observer = new HeadQuarterObserver();
	...
	System.out.println("HeadQuarter TERMINATES");
	try {
	 	observer.terminate();
	...
	protected void ask_setConnChannel_space()  {
		try{
	  		ILindaLike support = PlatformExpert.findOutSupport("space","setConnChannel","headQuarter",view);
		 	RunTimeKb.addSubjectConnectionSupport("headQuarter", support, view );
		 	String msgOut = "space_setConnChannel(headQuarter, setConnChannel, connect, 0) "  ;
			support.out( msgOut );
	/*		
			...
			cr.start();
	*/	
			IAcquireOneReply answer = new AcquireOneReply("headQuarter", "space","setConnChannel",core, 
				"headQuarter"+"_space_setConnChannel(space,setConnChannel,M,0)",view);
	 ...
	 protected void serveUpdateDispatchThread() throws Exception {
			new Thread(){
				...
				protected IMessage hl_node_serve_update(   ) throws Exception {
					IMessage m = new Message("headQuarter"+"_update"+"(ANY,update,M,N)");
					IMessage inMsg = comSup.inOnly( "headQuarter" ,"update", m );				
					return inMsg;				
				}		
		
				public void run(){
					System.out.println("headQuarter serveUpdateDispatchThread started");
					while(goon)
					try {
						IMessage m =  hl_node_serve_update();
	 					System.out.println("headQuarter storing content of: " + m   );
	...
	HeadQuarter system = new HeadQuarter( );
	system.doJob();
	}
}//HeadQuarterSupportMain
